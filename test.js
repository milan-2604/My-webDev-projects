...
myForm.addEventListener('submit', (e) => {
  ...
  if (changeTotal === totalCID) {
    // Status: CLOSED - return full drawer content in highest to lowest order
    ...
    // Status: OPEN - show only used change
    let openText = 'Status: OPEN';
    for (let [unit, amountUsed] of change) {
      openText += ` ${unit}: $${amountUsed.toFixed(2)}`;
    }
    changeDue.innerText = openText;

    // Subtract used amounts from cid
    for (let [unit, amountUsed] of change) {
      for (let i = 0; i < cid.length; i++) {
        if (cid[i][0] === unit) {
          cid[i][1] = round(cid[i][1] - amountUsed);
          break;
        }
      }
    }
